 ...
 *
 */
public class IndividualUniformPrior<N extends Node> implements MHWeight<RoundTrip<N>>, Prior {

	private final double[] uniformLogWeightsOverSize;

	// -------------------- CONSTRUCTION --------------------
	
	public IndividualUniformPrior(int numberOfNodes, int numberOfTimeBins) {
		this.uniformLogWeightsOverSize = new PriorUtils().computeUniformLogWeights(numberOfNodes, numberOfTimeBins);
	}

	public IndividualUniformPrior(Scenario<N> scenario) {
