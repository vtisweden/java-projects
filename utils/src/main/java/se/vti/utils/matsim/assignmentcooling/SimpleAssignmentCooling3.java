 ...
 */
@Singleton
public class SimpleAssignmentCooling3 implements IterationStartsListener, StartupListener {

	private static final Logger log = LogManager.getLogger(SimpleAssignmentCooling3.class);
	...
	@Inject
	public SimpleAssignmentCooling3(Config config) {
	...
	public void notifyIterationStarts(IterationStartsEvent event) {
		...
		if (iteration >= 0) {
			...
			for (String subpopulation : this.allSubpopulations) {
				...
				final double selectRandomRate = 1.0 - totalInnovationRate;// totalInnovationRate *
																			// this.maxNumberOfPlans;
				final double keepLastSelectedRate = 0.0; // Math.max(0.0, 1.0 - totalInnovationRate - selectRandomRate);
